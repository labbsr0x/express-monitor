 ...
 */
function init(app, shouldCollectDefaultMetrics, buckets, version, isErrorCallback) {
    if (!isCollectingMetrics && app) { 
        ...
        app.all(/^(?!\/metrics$).*/, (req, res, next) => {
            ...
            res.once("finish", () => {
                ...
                end({"type": "http", "status": res.statusCode, "method": req.method, "addr": req.url, "version": version, "isError": isErr}) // observes the request duration
                respSize.inc({"type": "http", "status": res.statusCode, "method": req.method, "addr": req.url, "version": version, "isError": isErr}, getContentLength(res)) // observes the request response size
            });
        })
