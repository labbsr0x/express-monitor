...
function init(app, shouldCollectDefaultMetrics, buckets) {
    if (!isCollectingMetrics) { 
        ...
        app.all(/^(?!\/metrics$).*/, (req, res, next) => {
            ...
            next();
            res.once("finish", () => {
                end({"type": "http", "status": res.statusCode, "method": req.method, "addr": req.url}) // observes the request duration
                respSize.inc({"type": "http", "status": res.statusCode, "method": req.method, "addr": req.url}, getContentLength(res)) // observes the request response size
            });
        })
